...
# Files to store visitor counts
TOTAL_VISITORS_FILE = "total_visitors.json"
UNIQUE_VISITORS_FILE = "unique_visitors.json"


# Ensure the necessary files exist
if not os.path.exists(TOTAL_VISITORS_FILE):
    with open(TOTAL_VISITORS_FILE, "w") as f:
        json.dump({"total_visits": 0}, f)

if not os.path.exists(UNIQUE_VISITORS_FILE):
    with open(UNIQUE_VISITORS_FILE, "w") as f:
        json.dump({"unique_visitors": []}, f)


# Load and save total visitors
def load_total_visitors():
    with open(TOTAL_VISITORS_FILE, "r") as f:
        return json.load(f)

def save_total_visitors(data):
    with open(TOTAL_VISITORS_FILE, "w") as f:
        json.dump(data, f, indent=4)


# Load and save unique visitors
def load_unique_visitors():
    with open(UNIQUE_VISITORS_FILE, "r") as f:
        return json.load(f)

def save_unique_visitors(data):
    with open(UNIQUE_VISITORS_FILE, "w") as f:
        json.dump(data, f, indent=4)


# Endpoint to count total and unique visits
@app.get("/count_visits")
async def count_visits(request: Request):
    # Increment total visits
    total_visitors_data = load_total_visitors()
    total_visitors_data["total_visits"] += 1
    save_total_visitors(total_visitors_data)

    # Track unique visitors by IP address (or use a better method, like cookies)
    unique_visitors_data = load_unique_visitors()
    visitor_ip = request.client.host

    # Hash the visitor's IP for privacy (optional)
    hashed_ip = hashlib.sha256(visitor_ip.encode()).hexdigest()

    if hashed_ip not in unique_visitors_data["unique_visitors"]:
        unique_visitors_data["unique_visitors"].append(hashed_ip)
        save_unique_visitors(unique_visitors_data)

    return {
        "total_visits": total_visitors_data["total_visits"],
        "unique_visitors": len(unique_visitors_data["unique_visitors"])
    }


# Files to store the counters
CERTS_CREATED_FILE = "certs_created.json"
FILES_WATERMARKED_FILE = "files_watermarked.json"
FILES_VERIFIED_FILE = "files_verified.json"

# Ensure the necessary files exist
for counter_file in [CERTS_CREATED_FILE, FILES_WATERMARKED_FILE, FILES_VERIFIED_FILE]:
    if not os.path.exists(counter_file):
        with open(counter_file, "w") as f:
            json.dump({"count": 0}, f)

# Helper functions to load and save counters
def load_counter(file_name):
    with open(file_name, "r") as f:
        return json.load(f)

def save_counter(file_name, data):
    with open(file_name, "w") as f:
        json.dump(data, f, indent=4)

# Endpoint to retrieve all counters for certs, watermarked files, and verified files
@app.get("/get_counters")
async def get_counters():
    certs_created = load_counter(CERTS_CREATED_FILE)["count"]
    files_watermarked = load_counter(FILES_WATERMARKED_FILE)["count"]
    files_verified = load_counter(FILES_VERIFIED_FILE)["count"]
    
    return {
        "certs_created": certs_created,
        "files_watermarked": files_watermarked,
...
def register_public_key(req: PublicKeyRegistration):
    ...
    # After the transaction is successfully broadcast
    try:
        # Increment certs created counter
        cert_counter = load_counter(CERTS_CREATED_FILE)
        cert_counter["count"] += 1
        save_counter(CERTS_CREATED_FILE, cert_counter)

    except Exception as e:
        logging.error(f"Error updating certs created counter: {str(e)}")
...
def finalize_watermark(req: FinalizeWatermarkRequest):
    ...
    for record in records[req.cert_id]:
        ...
        if record["original_file_hash"] == req.original_file_hash:
            ...
            if req.final_file_hash != req.original_file_hash:
                # This is the second call; create the final OP_RETURN transaction

                if not txid:
                    logging.error("Txid not provided in the second call")
                    raise HTTPException(status_code=400, detail="Txid not provided in the second call")

                # Create the final OP_RETURN transaction
                try:
                    ...
                    # Check if it's the second (final) call by ensuring final_file_hash is different
                    if req.final_file_hash != req.original_file_hash:
                        # Only increment the watermark counter when final_file_hash is different
                        try:
                            watermark_counter = load_counter(FILES_WATERMARKED_FILE)
                            watermark_counter["count"] += 1
                            save_counter(FILES_WATERMARKED_FILE, watermark_counter)
                        except Exception as e:
                            logging.error(f"Error updating files watermarked counter: {str(e)}")
                ...
                # Update the record with the final file's info
                ...
                # This is the first call; create the initial OP_RETURN transaction
                if not txid:
                    try:
                        data_values = [
                            cert_txid,
        ...
        for record in file_list:
            if req.file_hash == record.get("final_file_hash", ""):
                ...
                if req.file_size == record["final_file_size"]:
                    ...
                    # Increment verify counter on successful verification
                    try:
                        counter = load_counter(FILES_VERIFIED_FILE)  # Using the existing counter functions
                        counter["count"] += 1
                        save_counter(FILES_VERIFIED_FILE, counter)   # Save the incremented count
                    except Exception as e:
                        logging.error(f"Error updating verify counter: {str(e)}")
                        raise HTTPException(status_code=500, detail="Error updating verify counter")
...
@app.get("/find_signed_txid/{txid}")
async def find_signed_txid(txid: str):
    try:
        # Fetch transaction info to get the block hash
        tx_info = call_rpc('getrawtransaction', [txid, 1])
        block_hash = tx_info['blockhash']

        # Fetch the block by its hash
        block_info = call_rpc('getblock', [block_hash])
        transactions = block_info['tx']

        # Debug: print out the fetched block hash and transactions in that block
        print(f"Block Hash: {block_hash}")
        print(f"Transactions in block: {transactions}")

        # Search through transactions in the block
        signed_txid = None
        for tx in transactions:
            raw_tx = call_rpc('getrawtransaction', [tx, 1])
            print(f"Inspecting transaction: {raw_tx['txid']}")

            # Check all vout's for OP_RETURN with the original txid
            for vout in raw_tx['vout']:
                print(f"Inspecting vout: {vout}")
                
                # Check if 'asm' field contains OP_RETURN data
                if 'asm' in vout['scriptPubKey'] and 'OP_RETURN' in vout['scriptPubKey']['asm']:
                    op_return_data = vout['scriptPubKey']['asm'].split(' ')[1]  # Extract OP_RETURN data

                    # Decode the OP_RETURN data from hex to ASCII
                    op_return_ascii = bytes.fromhex(op_return_data).decode('utf-8')
                    print(f"OP_RETURN ASCII: {op_return_ascii}")

                    # Check if the OP_RETURN contains the original txid
                    if txid in op_return_ascii:
                        # If it does, the current transaction ID is our signed txid
                        signed_txid = raw_tx['txid']
                        print(f"Signed txid found: {signed_txid}")
                        break
        ...
        if not signed_txid:
            # Raise an error if no signed txid was found in the block
            print("Signed txid not found in the block")
            raise HTTPException(status_code=404, detail="Signed txid not found in the block")

        # Return the signed txid
        return {"signed_txid": signed_txid}

    except Exception as e:
        # Catch any errors and return a 500 status with error details
        print(f"Error finding signed txid: {str(e)}")
        raise HTTPException(status_code=500, detail=f"Error finding signed txid: {str(e)}")
