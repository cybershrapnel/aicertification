...
def register_public_key(req: PublicKeyRegistration):
    ...
    # Ensure that 'req.public_key' contains the private key you intend to clean
    clean_private_key = strip_pem_headers(req.public_key)

    # Now, embed the cleaned private key into OP_RETURN data
    op_return_data = f"{req.cert_id}|{username}|{clean_private_key}|{cert_level}"
...
def verify_watermark(req: WatermarkRequest):
    ...
    watermark = f"NanoCheeze AI Certification Watermark: CertID: {req.cert_id}, Signed Hash: {req.signature}"
...
def finalize_watermark(req: FinalizeWatermarkRequest):
        ...
        if record["original_file_hash"] == req.original_file_hash:
            ...
            if txid:
                ...
                try:
                    ...
                    # Prepare data for blockchain as pipe-separated values
                    data_values = [
                        req.cert_id,
                        record.get("username", ""),
    ...
    return {"message": "Final file hash and size recorded successfully, data written to blockchain", "txid": txid}
